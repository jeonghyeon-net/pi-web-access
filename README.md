...
Search for code examples, documentation, and API references via Exa MCP. No API key required. If Exa's dedicated `get_code_context_exa` MCP tool is unavailable, the extension falls back to `web_search_exa` + `web_fetch_exa` automatically.
